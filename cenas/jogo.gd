...
# Guarda a direção do último rio gerado para evitar padrões impossíveis pro jogador
...
func _ready() -> void:
	# randomize() sorteia uma semente (seed) aleatória no início, senão os rios viriam no mesmo lugar sempre
	randomize()
	
	# Busca o jogador dinamicamente na cena pela aba Groups
	player_nodo = get_tree().get_first_node_in_group("player")
	
	# Conecta o sinal do jogador para atualizar a interface (UI) aqui no nível root da cena
	if player_nodo != null:
		player_nodo.pontuacao_atualizada.connect(atualizar_texto_score)
	
	# Configura o ponto inicial da construção lá atrás no Z = 8 positivo
	proximo_z = 8
	
	# Gramas Escuras (Z = 8, 7, 6, 5)
	for i in range(4):
		spawnar_proxima_faixa_especifica(cena_grama_escura)
		
	# Gramas Normais de Recuo e Spawn (Z = 4, 3, 2, 1, 0)
	for i in range(5):
		spawnar_proxima_faixa_especifica(cena_grama)
		
	# Gramas Normais da Frente (Z = -1, -2, -3, -4, -5)
	for i in range(5):
		spawnar_proxima_faixa_especifica(cena_grama)
		
	# Preenche o resto da tela com o mapa infinito a partir de Z = -6, -7...
...
func _process(_delta: float) -> void:
	...
	# Se o player chegou perto do limite de chão construído...
	if linha_atual_player > (proximo_z_para_linha() - quantidade_faixas_na_tela):
		gerar_logica_procedural_faixa() # Cria chão na frente
		
		# Limpeza pra não afogar a memória RAM, arranca o primeiro elemento (o mais velho) da lista
		if faixas_vivas.size() > quantidade_faixas_na_tela + 10:
			...
			if is_instance_valid(faixa_antiga):
				faixa_antiga.queue_free() # Mata a instância!

	# SE TENTAR VOLTAR 4 PASSOS, MORRE
...
func gerar_logica_procedural_faixa() -> void:
	...
	match ultimo_tipo:
		...
		"picles":
			var escolha = randi() % 2
			if escolha == 0:
				# O próximo rio TEM que ser o oposto do rio anterior ao picles, pra ter como cruzar
...
func spawnar_faixa_na_posicao(cena: PackedScene, inverter: bool) -> void:
	...
	# .instantiate() tira da gaveta e vira um objeto real na memória
	var nova_faixa = cena.instantiate() as Node3D
	add_child(nova_faixa) # Tem que jogar na cena senão não renderiza
	
	nova_faixa.global_position = Vector3(0.0, 0.0, float(proximo_z))
	
	if inverter:
		# Girar 180 graus pra mudar o sentido dos carros/baguetes
		nova_faixa.global_rotation.y = deg_to_rad(180)
		
	faixas_vivas.append(nova_faixa)
	# Diminui o Z porque o player tá correndo pro fundo negativo
	proximo_z -= 1
